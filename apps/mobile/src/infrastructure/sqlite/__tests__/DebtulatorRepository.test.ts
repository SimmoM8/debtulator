...
describe('DebtulatorRepository transactions', () => {
  it('keeps underlying SQLite transactions serialized for adapter callers', async () => {
  ...
  it('exposes the transaction-scoped repository as the unit of work', async () => {
    const { database } = createDatabaseDouble();
    const repository = new DebtulatorRepository(database);

    await expect(
      repository.transaction(async (unitOfWork) => {
        expect(unitOfWork).toBe(repository);
        return 'committed';
      }),
