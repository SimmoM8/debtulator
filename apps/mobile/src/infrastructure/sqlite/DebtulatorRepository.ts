...
import { buildAccountDeletionPlan } from '@debtulator/application/data/accountDeletion';
import {
  canApplyRemoteSnapshot,
  getConflictResolutionAvailability,
  getRelatedSyncQueueEntries,
} from '@debtulator/domain/sync/conflictResolution';
import { buildRestorePlan, type RestoreResult } from '@debtulator/application/data/backupRestore';
...
} from '@debtulator/domain/groups/duplicates';
import type {
  AppSettings,
...
import { createId, nowIso, todayIsoDate } from '@debtulator/domain/shared/identifiers';
...
export class DebtulatorRepository {
  ...
  /**
   * Runs one application command atomically on every supported target.
   * Expo's exclusive transaction API is unavailable on web, so the shared
   * repository uses the cross-platform transaction API and keeps command
   * callbacks sequential and scoped to this database instance.
   */
  async transaction<T>(operation: (repository: DebtulatorRepository) => Promise<T>): Promise<T> {
