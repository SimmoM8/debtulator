...
import { canRetrySyncEntry } from '@debtulator/application/sync/syncPolicy';
import { addTelemetryBreadcrumb, captureTelemetryException, trackFirstSuccess, trackTelemetryEvent } from '@debtulator/application/observability/telemetry';
import {
...
import { pullRemoteData } from '@/src/infrastructure/supabase/sync/pullRemote';
import type {
  Attachment,
  Comment,
  Debt,
...
import { createId, nowIso } from '@debtulator/domain/shared/identifiers';

export type SyncEngineStore = AppSnapshot & {
...
async function createGroup(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateGroup(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const group = requiredLocal(snapshot.groups, entry.entityId, 'group');
  if (!group.remoteId) {
...
async function createGroupMember(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateGroupMember(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const member = requiredLocal(snapshot.sharedGroupMembers, entry.entityId, 'group member');
  if (!member.remoteId) {
    return createGroupMember(snapshot, store, entry);
  }
...
async function createInvite(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateInvite(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
  const invite = requiredLocal(snapshot.groupInvites, entry.entityId, 'group invite');
  if (!invite.remoteId) {
    return createInvite(snapshot, store, entry);
  }
...
async function createClaim(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateClaim(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
  const claim = requiredLocal(snapshot.groupMemberClaims, entry.entityId, 'group member claim');
  if (!claim.remoteId) {
    return createClaim(snapshot, store, entry);
  }
...
async function createSharedExpense(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateSharedExpense(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const expense = requiredLocal(snapshot.sharedExpenses, entry.entityId, 'shared expense');
  if (!expense.remoteId) {
    return createSharedExpense(snapshot, store, entry);
  }
...
async function createDebt(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
...
async function updateDebt(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const debt = requiredLocal(snapshot.debts, entry.entityId, 'debt');
  if (debt.visibility !== 'shared_with_involved_member') {
    return markEntitySynced(snapshot, store, 'debt', debt.id);
  }
...
async function createGroupDebt(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateGroupDebt(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const debt = requiredLocal(snapshot.groupDebts, entry.entityId, 'group debt');
  if (!debt.remoteId) {
    return createGroupDebt(snapshot, store, entry);
  }
...
async function upsertGroupVerification(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
...
async function createPayment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updatePayment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const payment = requiredLocal(snapshot.payments, entry.entityId, 'payment');
  if (!payment.remoteId) {
    return createPayment(snapshot, store, entry);
  }
...
async function createSettlement(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateSettlement(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry, userId: string) {
  const settlement = requiredLocal(snapshot.settlements, entry.entityId, 'settlement');
...
async function createComment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateComment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
  const comment = requiredLocal(snapshot.comments, entry.entityId, 'comment');
  if (!comment.remoteId) {
    return createComment(snapshot, store, entry);
  }
...
async function createAttachment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
...
async function updateAttachment(snapshot: AppSnapshot, store: SyncEngineStore, entry: SyncQueueEntry) {
  const attachment = requiredLocal(snapshot.attachments, entry.entityId, 'attachment');
...
function hasRemoteIdForLocalId(snapshot: AppSnapshot, localId: string) {
...
function replace<K extends keyof AppSnapshot, T extends { id: string }>(
  snapshot: AppSnapshot,
  key: K,
  item: T,
) {
