...
import { AuthSessionCoordinator } from '@debtulator/application/auth/AuthSessionCoordinator';
import { SyncCoordinator } from '@debtulator/application/sync/SyncCoordinator';
import { DEFAULT_BASE_CURRENCY } from '@debtulator/domain/finance/currencies';
import { isSupabaseConfigured, supabase } from '@/src/infrastructure/supabase/client';
import { createSupabaseAuthSessionPort } from '@/src/infrastructure/supabase/authSessionPort';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const activeSessionUserIdRef = useRef<string | null>(user?.id ?? null);
  ...
  useEffect(() => {
    const coordinator = new SyncCoordinator(async ({ userId, signal }) => {
      const isCurrent = () =>
        !signal.aborted && activeSessionUserIdRef.current === userId;
      if (!isCurrent()) return;
      await refreshProfileRef.current();
      if (!isCurrent()) return;
      await syncStage2RecordsRef.current();
      if (!isCurrent()) return;
      await runRemoteDataSyncRef.current();
    });
  ...
  useEffect(() => {
    ...
    const coordinator = new AuthSessionCoordinator(
      createSupabaseAuthSessionPort(supabase),
    );
    const stop = coordinator.start((nextSession) => {
  ...
  useEffect(() => {
    ...
    syncCoordinatorRef.current?.setUser(user?.id ?? null);
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    if (user?.id && data.ready && bootSyncedUserIdRef.current !== user.id) {
      ...
      syncCoordinatorRef.current?.request('auth-bootstrap').catch(() => undefined);
    }
  }, [data.ready, user?.id]);
  ...
  useEffect(() => {
    ...
    handledResetVersionRef.current = data.syncedDataResetVersion;
    syncCoordinatorRef.current?.request('reset').catch(() => undefined);
  ...
  useEffect(() => {
    ...
    const scheduleLinkRequestSync = () => {
      ...
      syncTimer = setTimeout(() => {
        syncCoordinatorRef.current?.request('realtime').catch(() => undefined);
      }, 50);
    };
    ...
    const unsubscribeFromForeground = subscribeToAppForeground(() => {
      syncCoordinatorRef.current?.request('foreground').catch(() => undefined);
    });
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      syncCoordinatorRef.current?.request('outbox').catch(() => undefined);
    }, 500);
  ...
  const refreshSync = useCallback(async () => {
    ...
    try {
      ...
      await syncCoordinatorRef.current?.request('manual');
