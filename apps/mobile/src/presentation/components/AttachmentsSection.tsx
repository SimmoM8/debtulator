...
import { Badge } from '@/src/presentation/design-system/Badges';
import { Button, Card, EmptyState, SectionTitle, SelectChips, TextField } from '@/src/presentation/design-system/Primitives';
...
} from '@debtulator/domain/attachments/attachments';
...
import type { Attachment, AttachmentKind, AttachmentTargetType } from '@debtulator/domain/models';
...
export function AttachmentsSection({
  ...
  async function addAttachment() {
    ...
    if (
      ...
      try {
        const remote = await collaboration.attachments.upload(attachment);
        if (remote) {
  ...
  async function pickDocument() {
    try {
      const result = await platform.documents.pickDocument({
        mimeTypes: ['application/pdf', 'image/*', 'text/csv', 'text/plain'],
      });
      if (result.status !== 'selected') {
        return;
      }
      const asset = result.file;
      await setPickedFile({
        uri: asset.uri,
  ...
  async function pickImage() {
    try {
      const result = await platform.media.pickImage();
  ...
  async function setPickedFile(file: PickedAttachment) {
    ...
    if (file.fileSize <= 0) {
      try {
        fallbackSize = (await platform.files.getInfo(file.uri)).size;
      } catch {
        fallbackSize = 0;
      }
    }
    const fileWithSize = {
      ...file,
      fileSize: file.fileSize || fallbackSize,
    };
  ...
  return (
    <View style={styles.attachmentRow}>
      <Pressable
        ...
        onPress={() => router.push(routes.attachmentDetail(attachment.id))}
