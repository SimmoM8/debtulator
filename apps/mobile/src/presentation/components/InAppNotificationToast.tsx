...
import { notificationEnabled, privacySafeNotificationBody } from "@debtulator/domain/notifications/notificationPolicy";
import { useAppData } from "@/src/presentation/providers/AppDataProvider";
import type { AppNotification, CurrencyCode, DebtDirection } from "@debtulator/domain/models";
...
export function openNotificationTarget(
  ...
  if (notification.targetType === "sync_conflict" && notification.targetId) {
    router.push(routes.conflictDetail(notification.targetId));
    return;
  }
  ...
  if (notification.type === "payment") {
    ...
    router.push(kind?.includes("confirmation") ? routes.requests() : routes.notifications());
    return;
  }
  ...
  if (notification.targetId) {
    switch (notification.targetType) {
      case "member":
        router.push(routes.memberDetail(notification.targetId));
        return;
      case "group":
        router.push(routes.groupDetail(notification.targetId));
        return;
      case "settlement":
        router.push(routes.debtSettlementDetail(notification.targetId));
        return;
      case "attachment":
        router.push(routes.attachmentDetail(notification.targetId));
        return;
      case "shared_expense":
        router.push(routes.expenseDetail(notification.targetId));
        return;
      case "sync_conflict":
        router.push(routes.conflictDetail(notification.targetId));
        return;
