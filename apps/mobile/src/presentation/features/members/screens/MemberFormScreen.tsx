...
import type { SignedUpMemberProfile } from "@debtulator/application/ports/collaborationGateway";
...
export function MemberFormScreen() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      collaboration.memberDirectory.searchProfiles({
        query,
  ...
  }, [auth.identity.authenticatedUserId, collaboration, member, profileQuery]);
      ...
      if (member) {
        ...
        const acceptedLink = selectedProfile
          ? acceptedLinkInLocalData ||
            (await collaboration.memberLinks.hasAcceptedLink(selectedProfile.id))
          : false;
        ...
        if (acceptedLink && selectedProfile && created.linkStatus !== "linked") {
          ...
          const remoteId =
            ...
            (await collaboration.memberLinks.createRequest({
              requesterUserId,
