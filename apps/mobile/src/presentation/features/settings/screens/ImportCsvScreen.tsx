...
import { previewCsvImport, type ImportPreviewRow } from "@debtulator/application/data/csv";
import { fileNameFromUri, isSupportedCsvFile } from "@debtulator/domain/attachments/attachments";
...
export function ImportCsvScreen() {
  ...
  async function pickCsvFile() {
    try {
      ...
      const text = await platform.files.readText(asset.uri);
      setCsvText(text);
      setSourceName(name);
      setFileFeedback(
