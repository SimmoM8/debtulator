...
import { DebtulatorOrbitIllustration } from "@/src/presentation/components/illustrations/DebtulatorOrbitIllustration";
import {
    Button,
    Card,
    LoadingState,
...
import {
    ...
    settlementsToCsv,
    tagsToCsv,
} from "@debtulator/application/data/csv";
import {
    groupPdfLines,
    groupTextSummary,
    shareExport,
...
import { explainGroupSettlement } from "@debtulator/domain/ledger/ledger";
import {
  addTelemetryBreadcrumb,
  captureTelemetryException,
  trackTelemetryEvent,
} from "@debtulator/application/observability/telemetry";
...
export function ExportDataScreen() {
  ...
  async function exportCsv() {
    try {
      ...
      const uri = await writeTextExport(platform.files, buildExportFilename(scope), csv);
      ...
      await shareExport(platform.sharing, uri, `Debtulator ${scope} CSV`);
    } catch (error) {
  ...
  async function exportFirstGroupPdf() {
    ...
    try {
      ...
      await shareExport(platform.sharing, uri, `${group.name} PDF summary`);
    } catch (error) {
  ...
  async function shareFirstGroupText() {
    ...
    try {
      ...
      await shareExport(platform.sharing, uri, `${group.name} text summary`, text);
    } catch (error) {
      ...
      <PageHeader
        ...
        action={
          <Button
            ...
            icon="cloud-upload"
            variant="secondary"
            onPress={() => router.push(routes.importCsv())}
          />
        }
      />
