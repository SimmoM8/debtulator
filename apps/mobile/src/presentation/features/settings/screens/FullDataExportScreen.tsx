...
export function FullDataExportScreen() {
  ...
  async function exportData() {
    try {
      ...
      const uri = await platform.files.writeTextFile(
        `debtulator-full-export-${exportedAt.slice(0, 10)}.json`,
      ...
      await platform.sharing.shareFile({
        uri,
        message: "Debtulator full data export",
      });
    } catch (error) {
