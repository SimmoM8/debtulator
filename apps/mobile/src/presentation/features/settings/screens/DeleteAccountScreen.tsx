...
import { DebtulatorShieldIllustration } from "@/src/presentation/components/illustrations/DebtulatorShieldIllustration";
import {
  Button,
  Card,
  PageHeader,
...
import { palette, spacing, typefaces, typography } from "@/src/presentation/theme/design";
import { getLatestAccountDeletionState } from "@debtulator/application/data/accountDeletion";
import type { AccountDeletionRequest } from "@debtulator/application/ports/collaborationGateway";
...
export function DeleteAccountScreen() {
  ...
  const auth = useAuth();
  const { accountDeletion } = useCollaboration();
  ...
  const refreshRemoteStatus = useCallback(async () => {
    ...
    try {
      setStatusError(null);
      setRemoteRequest(await accountDeletion.fetchLatest(authenticatedUserId));
    } catch (error) {
      setStatusError(error instanceof Error ? error.message : "Unable to load deletion status.");
    }
  }, [accountDeletion, authenticatedUserId]);
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setSubmitting(true);
            try {
              const remote = await accountDeletion.request({
                deleteLocalData,
                keepLocalArchive,
