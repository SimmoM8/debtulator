...
import { AvatarStack, GlassCard } from "@/src/presentation/design-system/Finance";
import { ActivityTimelineRow } from "@/src/presentation/components/ActivityTimelineRow";
import { ConfirmationMarker } from "@/src/presentation/components/ConfirmationMarker";
import { GlassSurface } from "@/src/presentation/design-system/GlassSurface";
import { MemberAvatar } from "@/src/presentation/design-system/MemberAvatar";
import { MobileMenuModal } from "@/src/presentation/design-system/MenuList";
import { Amount } from "@/src/presentation/design-system/Money";
import { TagInput } from "@/src/presentation/design-system/TagInput";
import {
  Button,
  Card,
  DatePickerField,
...
import { buildLedgerEntries } from "@debtulator/domain/ledger/ledger";
...
import { todayIsoDate } from "@debtulator/domain/shared/identifiers";
...
export function DebtDetailScreen() {
  ...
  async function persistStatusProposal(
    ...
    try {
      if (counteredVerification) {
        ...
        const remote = await collaboration.debtVerifications.counter({
      ...
      const remote = await collaboration.debtVerifications.create({
        debt: local.debt,
        member,
        requesterUserId: currentUserId,
        responderUserId: member.linkedUserId,
  ...
  async function exportPdf() {
    ...
    await shareExport(platform.sharing, uri, `${currentDebt.title} PDF`);
  }
  ...
  async function sendConfirmationReminder(verification: DebtVerification) {
    ...
    try {
      await collaboration.debtVerifications.sendReminder({
  ...
  async function sendPaymentReminder(payment: Payment) {
    ...
    try {
      await collaboration.paymentConfirmations.sendReminder({
        paymentRemoteId: payment.remoteId,
      });
      ...
      footer={
        <View style={styles.footerActions}>
          ...
          <Button
            ...
            variant="secondary"
            onPress={() =>
              router.push(routes.paymentForm({ debtId: currentDebt.id }))
            }
            style={styles.footerButton}
          />
        </View>
            ...
            items: [
              {
                ...
                onPress: () => {
                  ...
                  router.push(routes.debtForm({ id: currentDebt.id }));
                },
              },
              {
                label: "Export PDF",
