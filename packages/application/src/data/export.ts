...
import { formatMoney, formatMoneyMap } from '@debtulator/domain/finance/money';
...
export async function writeTextExport(files: FileGateway, fileName: string, text: string) {
  try {
    const uri = await files.writeTextFile(safeFileName(fileName), text);
...
export async function writePdfExport(files: FileGateway, fileName: string, lines: string[]) {
  try {
    const uri = await files.writeTextFile(
...
export async function shareExport(sharing: ShareGateway, uri: string, title: string, message?: string) {
  try {
    await sharing.shareFile({ title, message: message ?? title, uri });
