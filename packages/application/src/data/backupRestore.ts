...
import { addTelemetryBreadcrumb, captureTelemetryException, trackTelemetryEvent } from '@debtulator/application/observability/telemetry';
import { sanitizeAttachmentsForPortableExport } from '@debtulator/application/data/export';
import type { FileGateway, ShareGateway } from '@debtulator/application/ports/fileGateway';
import type {
  AppSettings,
  Attachment,
  AuditLog,
...
export function buildBackup(snapshot: AppSnapshot, options: BackupOptions): DebtulatorBackup {
...
export async function shareBackupFile(
  ...
  backup: DebtulatorBackup,
) {
  try {
    const fileUri = await files.writeTextFile(
      `debtulator-backup-${backup.exportedAt.slice(0, 10)}.json`,
      JSON.stringify(backup, null, 2),
    );
    await sharing.shareFile({
      uri: fileUri,
      message: 'Debtulator backup export',
    });
...
export function buildRestorePlan(rawJson: string, current: AppSnapshot, mode: BackupMode): RestoreApplyPlan {
...
function buildExistingIds(snapshot: AppSnapshot): ExistingIds {
  return {
