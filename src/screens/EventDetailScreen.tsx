...
import { DEFAULT_EVENT_SETTLEMENT_SETTINGS, entriesForEvent, explainEventSettlement, participantName } from '@/src/services/ledger';
import { createRemoteEventInvite, createRemoteSharedEventMember } from '@/src/services/stage3Sync';
...
export function EventDetailScreen() {
  ...
  const [settlementSettings, setSettlementSettings] = useState<EventSettlementSettings>(DEFAULT_EVENT_SETTLEMENT_SETTINGS);
  const [newMemberName, setNewMemberName] = useState('');
  const [newMemberEmail, setNewMemberEmail] = useState('');
  const [newMemberPhone, setNewMemberPhone] = useState('');
  const [inviteDisplayName, setInviteDisplayName] = useState('');
  const [inviteEmail, setInviteEmail] = useState('');
  const [inviteRole, setInviteRole] = useState<Exclude<EventRole, 'owner'>>('member');
  const [claimMessage, setClaimMessage] = useState('');
  const [rejectionReasons, setRejectionReasons] = useState<Record<string, string>>({});

  const currentUserId = auth.identity.authenticatedUserId;
  const participant = useMemo(
    () => (event ? participantForUser(event, data.eventParticipants, currentUserId) : null),
    [currentUserId, data.eventParticipants, event],
  );
  const permissionContext = event ? { event, participant, userId: currentUserId } : null;
  ...
  const activeSharedMembers = sharedEventMembers.filter((member) => member.status !== 'archived' && member.status !== 'merged');
  const currentEventMember = sharedEventMembers.find(
    (member) => currentUserId && member.linkedUserId === currentUserId && member.status !== 'merged',
  );
  ...
  async function updateStatus(status: EventStatus) {
    if (status === 'finalising') {
      Alert.alert('Finalise event', 'Finalised events are locked for normal editing.', [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Finalise event', onPress: () => data.updateEvent(currentEvent.id, { status }) },
      ]);
      return;
    }
    await data.updateEvent(currentEvent.id, {
      status,
      archived: status === 'archived' ? true : status === 'active' ? false : currentEvent.archived,
    });
  }
  ...
  async function addUnlinkedEventMember() {
    ...
    try {
      ...
      if (remoteId) {
        await data.upsertSharedEventMember({ ...member, remoteId, syncStatus: 'synced' });
      }
    } catch {
      await data.upsertSharedEventMember({ ...member, syncStatus: 'sync_error' });
    }
    setNewMemberName('');
    setNewMemberEmail('');
    setNewMemberPhone('');
  ...
  async function sendInvite() {
    ...
    try {
      ...
      if (remoteId) {
        await data.upsertEventInvite({ ...invite, remoteId, syncStatus: 'synced' });
      }
    } catch {
  ...
  return (
    <Screen>
      <PageHeader
        ...
        title={event.name}
        subtitle={event.notes ?? 'Collaborative expenses, balances, settlements, and activity.'}
        action={
          canManagePeople || !isShared ? (
            <IconButton icon="create-outline" label="Edit event" onPress={() => router.push({ pathname: '/event/form', params: { id: event.id } })} />
          ) : undefined
        }
      />
      ...
      <Card tone={isShared ? 'mint' : 'blue'}>
        <View style={styles.eventTop}>
          <View style={styles.flexOne}>
            <View style={styles.badgeLine}>
              ...
              <Badge label={isShared ? 'shared event' : 'private event'} tone={isShared ? 'blue' : 'neutral'} />
              <Badge label={role} tone={role === 'viewer' ? 'neutral' : 'positive'} />
              <SyncBadge status={event.syncStatus} />
              {event.lockedAt || event.finalisedAt ? <Badge label="locked" tone="amber" /> : null}
            </View>
            <Text style={styles.label}>Your event balance</Text>
            <BalanceStack
              balances={(myBalanceId ? explanation.participantNets[myBalanceId] : undefined) ?? {}}
          ...
          <View style={styles.badgeBox}>
            <Text style={styles.badgeNumber}>{isShared ? activeSharedMembers.length : eventMemberIds.length + 1}</Text>
        ...
        <View style={styles.actionRow}>
          <Button
            ...
            onPress={() => router.push({ pathname: '/expense/form', params: { eventId: event.id } })}
          />
          <Button
      ...
      {tab === 'overview' ? (
        <>
          <Card>
            <SectionTitle title="Event status" subtitle="Lifecycle controls respect role and lock state." />
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Visibility</Text>
              <Text style={styles.infoValue}>{isShared ? 'Shared/synced' : 'Private/local'}</Text>
            </View>
            <View style={styles.infoRow}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>{explanation.includedEntries.length}</Text>
            </View>
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>Excluded by settings</Text>
              <Text style={styles.infoValue}>{explanation.excludedEntries.length}</Text>
            </View>
            <View style={styles.actionRow}>
              {canFinaliseEvent(permissionContext) ? (
                <Button title="Finalise event" icon="lock-closed" variant="secondary" onPress={() => updateStatus('finalising')} />
              ) : null}
              {canReopenEvent(permissionContext) ? (
                <Button title="Reopen event" icon="lock-open" variant="secondary" onPress={() => updateStatus('active')} />
              ) : null}
              {canFinaliseEvent(permissionContext) ? (
                <Button title="Mark settled" icon="checkmark-circle" variant="secondary" onPress={() => updateStatus('settled')} />
              ) : null}
              {canArchiveEvent(permissionContext) ? (
                <Button title="Archive event" icon="archive" variant="danger" onPress={() => updateStatus('archived')} />
              ) : null}
            </View>
          </Card>

          {isShared && (pendingInvites.length > 0 || pendingClaims.length > 0 || sharedDuplicateWarnings.length > 0) ? (
            <Card tone="amber">
              <SectionTitle title="Needs attention" subtitle="Invites, claims, duplicate warnings, and disputed records." />
              <View style={styles.countLine}>
                <Badge label={`${pendingInvites.length} pending invites`} tone="amber" />
                <Badge label={`${pendingClaims.length} claim requests`} tone="blue" />
                <Badge label={`${sharedDuplicateWarnings.length} duplicate warnings`} tone="negative" />
              </View>
            </Card>
          ) : null}
        </>
      ...
      {tab === 'expenses' ? (
        <Card>
          <SectionTitle title="Event ledger" subtitle="Expenses and direct event debts are verified at event level." />
          {eventEntries.length > 0 ? (
            eventEntries.map((entry) => (
              <EventLedgerRow
                ...
                onReasonChange={(value) => setRejectionReasons((current) => ({ ...current, [targetKey(entry)]: value }))}
                onVerify={() => {
            ...
            <EmptyState title="No event records" body="Add a shared expense or direct event debt." />
          )}
        </Card>
      ) : null}
      ...
      {tab === 'balances' ? (
        <Card>
          <SectionTitle title="Balances by currency" subtitle="Positive balances receive money; negative balances pay money." />
          {Object.entries(explanation.participantNets).map(([participantId, moneyMap]) => (
            <View key={participantId} style={styles.infoRow}>
              <Text style={styles.infoLabel}>{participantName(participantId, data.members, sharedEventMembers)}</Text>
      ...
      {tab === 'settlements' ? (
        <>
          <Card>
            <SectionTitle title="Settlement suggestions" subtitle="Generated separately by currency using a minimised transfer pass." />
            <SettingToggle
          ...
          <Card>
            <SectionTitle title="How this was calculated" subtitle="Included and excluded rows are shown with reasons." />
            <View style={styles.countLine}>
        ...
        isShared ? (
          <SharedMembersPanel
            ...
            ignoreWarning={(warningId) =>
              currentUserId ? data.ignoreEventDuplicateWarning(warningId, currentUserId) : undefined
            }
            mergeMembers={(sourceId, targetId) =>
              currentUserId ? data.mergeSharedEventMembers(sourceId, targetId, currentUserId) : undefined
            }
            approveClaim={(claimId) =>
              currentUserId ? data.respondToEventMemberClaim(claimId, 'approved', currentUserId) : undefined
            }
            rejectClaim={(claimId) =>
              currentUserId ? data.respondToEventMemberClaim(claimId, 'rejected', currentUserId) : undefined
            }
          />
        ) : (
          <PrivateMembersPanel
            ...
            ignoreWarning={(key) => data.updateEvent(event.id, { ignoredDuplicateKeys: [...event.ignoredDuplicateKeys, key] })}
          />
        )
      ) : null}
      ...
      {tab === 'activity' ? (
        <Card>
          <SectionTitle title="Activity" subtitle="Shared event changes are recorded for participants." />
          {eventActivity.length > 0 ? (
            eventActivity.map((activity) => (
              <View key={activity.id} style={styles.activityRow}>
                <Text style={styles.rowTitle}>{activity.action.replaceAll('_', ' ')}</Text>
                <Text style={styles.body}>{new Date(activity.createdAt).toLocaleString()}</Text>
              </View>
            ))
          ) : (
...
function EventLedgerRow({
  ...
  const currentMemberInvolved =
    currentEventMember && (entry.fromId === currentEventMember.id || entry.toId === currentEventMember.id);
  return (
    <View style={styles.verificationBlock}>
      ...
      <View style={styles.badgeLine}>
        ...
        <Text style={styles.body}>
          {participantName(entry.fromId, [], sharedMembers)} owes {participantName(entry.toId, [], sharedMembers)}
        </Text>
      </View>
      {canVerify && currentMemberInvolved ? (
        <>
          ...
          <View style={styles.actionRow}>
            ...
            <Button title="Reject" icon="close-circle" variant="danger" disabled={!rejectionReason.trim()} onPress={onReject} />
          </View>
        </>
      ) : null}
    </View>
      ...
      {[
        ...
        return (
          <Button
            ...
            onPress={() => onChange({ ...settings, [typedKey]: !settings[typedKey] })}
          />
        );
      })}
    </View>
...
function SharedMembersPanel({
  ...
  warnings: { id: string; eventMemberIdA: string; eventMemberIdB: string; reason: string; confidence: string }[];
  claims: { id: string; eventMemberId: string; claimantUserId: string; message: string | null }[];
  canManage: boolean;
  canInvite: boolean;
  ...
  const activeMembers = eventMembers.filter((member) => member.status !== 'merged');
  return (
    <>
      <Card>
        <SectionTitle title="Event members" subtitle="Linked users and shared placeholders are event-specific." />
        {activeMembers.map((member) => (
          <View key={member.id} style={styles.memberRow}>
            <View style={styles.flexOne}>
              <View style={styles.badgeLine}>
                <Text style={styles.rowTitle}>{member.displayName}</Text>
                <Badge label={member.type === 'linked_user' ? 'linked' : 'unlinked'} tone={member.type === 'linked_user' ? 'positive' : 'amber'} />
                <Badge label={member.status} tone={member.status === 'active' ? 'neutral' : 'blue'} />
              </View>
              {member.email || member.phone ? <Text style={styles.body}>{member.email ?? member.phone}</Text> : null}
            </View>
            {member.type === 'unlinked_placeholder' && currentUserId ? (
              <Button title="Claim this member" icon="person-add" variant="secondary" onPress={() => claimMember(member)} />
            ) : null}
          </View>
        ))}
        {currentUserId ? (
          <TextField label="Claim message" value={claimMessage} onChangeText={setClaimMessage} placeholder="Optional" />
        ) : null}
      </Card>

      {canManage ? (
        <Card>
          <SectionTitle title="Add unlinked member" subtitle="Shared placeholders are visible to all event participants." />
          <TextField label="Display name" value={newMemberName} onChangeText={setNewMemberName} placeholder="Dad" />
          <TextField label="Email" value={newMemberEmail} onChangeText={setNewMemberEmail} keyboardType="email-address" />
          <TextField label="Phone" value={newMemberPhone} onChangeText={setNewMemberPhone} keyboardType="phone-pad" />
          <Button title="Add unlinked member" icon="person-add" disabled={!newMemberName.trim()} onPress={addUnlinkedEventMember} />
        </Card>
      ) : null}

      {canInvite ? (
        <Card>
          <SectionTitle title="Invite members" subtitle="Accepting an invite shares only event-specific records." />
          <TextField label="Display name" value={inviteDisplayName} onChangeText={setInviteDisplayName} placeholder="Sarah" />
          <TextField label="Email" value={inviteEmail} onChangeText={setInviteEmail} keyboardType="email-address" />
          <SelectChips
            label="Role offered"
            value={inviteRole}
            options={[
          ...
          />
          <Button title="Invite members" icon="mail" disabled={!inviteDisplayName.trim()} onPress={sendInvite} />
        </Card>
      ) : null}

      {claims.length > 0 ? (
        <Card tone="blue">
          <SectionTitle title="Claim requests" subtitle="Owner/admin approval links a placeholder to a real user." />
          {claims.map((claim) => {
            const member = eventMembers.find((item) => item.id === claim.eventMemberId);
            return (
              <View key={claim.id} style={styles.warningRow}>
                <Text style={styles.body}>
                  {claim.claimantUserId} wants to claim {member?.displayName ?? 'an event member'}.
                </Text>
                {claim.message ? <Text style={styles.body}>{claim.message}</Text> : null}
                {canManage ? (
                  <View style={styles.actionRow}>
                    <Button title="Approve claim" icon="checkmark-circle" onPress={() => approveClaim(claim.id)} />
                    <Button title="Reject claim" icon="close-circle" variant="secondary" onPress={() => rejectClaim(claim.id)} />
                  </View>
                ) : null}
              </View>
            );
      ...
      {warnings.length > 0 ? (
        <Card tone="amber">
          <SectionTitle title="Possible duplicate member" subtitle="Warnings never auto-merge members." />
          {warnings.map((warning) => {
            const first = eventMembers.find((member) => member.id === warning.eventMemberIdA);
            const second = eventMembers.find((member) => member.id === warning.eventMemberIdB);
            return (
              <View key={warning.id} style={styles.warningRow}>
                <Text style={styles.body}>
                  {first?.displayName ?? 'Member'} and {second?.displayName ?? 'Member'} may refer to the same person. {warning.reason}
                </Text>
                <View style={styles.actionRow}>
                  <Button title="Ignore" icon="close" variant="secondary" onPress={() => ignoreWarning(warning.id)} />
                  {canManage && first && second ? (
                    <Button title="Merge" icon="git-merge" onPress={() => mergeMembers(first.id, second.id)} />
                  ) : null}
                </View>
              </View>
            );
...
function PrivateMembersPanel({
  ...
  return (
    <>
      <Card>
        <SectionTitle title="Private event members" subtitle="Private events use local/manual members only." />
      ...
      {duplicateWarnings.length > 0 ? (
        <Card tone="amber">
          <SectionTitle title="Possible duplicate members" subtitle="Warnings do not auto-merge local contacts." />
          {duplicateWarnings.map((warning) => (
            <View key={warning.key} style={styles.warningRow}>
              <Text style={styles.body}>{warning.message}</Text>
              <Button title="Ignore warning" icon="close" variant="secondary" onPress={() => ignoreWarning(warning.key)} />
            </View>
          ))}
        </Card>
      ) : null}
